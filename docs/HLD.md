...
- **Alert sources (all publish to `supervisor-alerts`):**
  ...
  - Agent offline mid-ticket (from heartbeat sweep, §9.4)
...
### 9.4 Liveness via heartbeat (silent-disconnect protection)

A crashed laptop or dropped network may not fire a clean WebSocket `close`, leaving Redis falsely showing the agent as available. Guard with a **heartbeat**: the dashboard pings every ~30s updating `agent:{id}:last_seen`; a background sweep marks any agent with `last_seen > 90s` as offline and **reassigns their active tickets**.
